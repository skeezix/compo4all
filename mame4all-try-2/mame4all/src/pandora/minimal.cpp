...
void pnd_sound_play(void *buff, int len)
{
	...
	SDL_LockAudio();
	memcpy( (char*)pnd_audio_spec.userdata + pnd_sndlen, buff, len ); // floopy
	pnd_sndlen += len;
	SDL_UnlockAudio();
}
...
static void pnd_sound_callback(void *data, Uint8 *stream, int len)
{
	...
	pnd_sndlen -= len;
	memcpy( data, (char*)data + len, pnd_sndlen ); // floopy
}
...
void pnd_set_video_mode(int bpp,int width,int height)
{
	...
	fb_ptr[1] = (unsigned short*)fb_ptr[0] + fb_mmap_size/2; // floopy
