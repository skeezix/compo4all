...
namespace HiGames
{
    class rtype2 : Hiscore
    {
        ...
        public rtype2()
        {
            ...
            m_gamesSupported = "rtype2,rtype2j";
        ...
        private byte[] ScoreToHiScoreLong(int score)
        {
            string scoreAsString = score.ToString().PadLeft(7, '0');
            bool hasHitReal = false;
            byte[] toReturn = new byte[scoreAsString.Length];

            for (int i = 0; i < scoreAsString.Length; i++)
            {
                if (scoreAsString[i].Equals('0'))
                {
                    if (hasHitReal)
                        toReturn[i] = 0x30;
                    else
                ...
                {
                    toReturn[i] = Convert.ToByte(scoreAsString[i]);
                    hasHitReal = true;
                }
            }
        ...
        public override void SetHiScore(string[] args)
        {
            ...
            switch (rank)
            {
                case 0:
                    ...
                    HiConvert.ByteArrayCopy(hiscoreData.HiScoreLong, ScoreToHiScoreLong(score));
                    break;
                case 1:
