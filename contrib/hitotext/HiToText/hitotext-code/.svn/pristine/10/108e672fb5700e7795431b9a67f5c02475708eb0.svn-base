...
namespace HiGames
{
    class robotron : Hiscore
    {
        [StructLayout(LayoutKind.Sequential, Pack = 1, CharSet = CharSet.Ansi)]
        public struct HiscoreData
        {
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 306)]
            public byte[] UnusedA;
        ...
        public robotron()
        {
            ...
            m_gamesSupported = "robotron,robotryo";
        ...
        public string ByteArrayToString(byte[] data)
        {
            ...
            for (int i = 0; i < data.Length / 2; i++)
            {
                ...
                if (tmpvalue == 0x3a) // If there is an 0x3a (0xf3, 0xfa) an space must be displayed.
                    sb.Append(' ');
                else
                    sb.Append((char)tmpvalue);
            }
        ...
        public byte[] StringToByteArray(string str, int length)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                if (str[i] == ' ') // An space must be coded like 0xf3, 0xfa
                  {
                    data[i * 2] = 0xf3;
                    data[(i * 2) + 1] = 0xfa;
                  }               
